...
def create_response_and_execute_reaction(packet: Packet, session_id, client_handler) -> Packet: # Session id is kept in the client handler on the server side and thus cannot be obtained from client's message
    ...
    def handle_request_with_file():
        ...
        if packet.body["args"]["file"] != "":
            ...
            is_recv, file_name = receive_file(client_handler.client_socket, packet.body["args"]["file"])
            file_type = file_name.split(".")[-1]
            if not is_recv:
                write_to_log("Error - file count not be transferred")

        result = add_request(packet.body["args"], session_id, file_name, file_type)
        if packet.body["args"]["file"] != "":
            os.remove(file_name)
        return result
    ...
    if cmd in SESSION_REQUESTS:
        is_cmd_allowed = handle_session_limit(session_token.session_code)
    ...
    if check_cmd(cmd) == 1:
        ...
        response, session_code, client_handler.session = login_with_old_session(session_token.session_code)
        resp_packet.mod_body("msg", response)
        resp_packet.mod_body("session", session_code)
    elif cmd == "Request":
        resp_packet.mod_body("msg", handle_request_with_file())
    elif cmd == "Delete_session":
        resp_packet.mod_body("msg", delete_session(session_id))
    elif cmd == "Songs":
        song_names = fetch_song_names(args)
        resp_packet.mod_body("msg", str(song_names))
